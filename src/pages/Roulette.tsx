...
import { Crown, RotateCcw, Loader2, Delete, CornerDownLeft } from "lucide-react";
  ...
  useEffect(() => {
    if (value.includes("@") && !value.includes("@.")) {
      ...
      if (afterAt && !afterAt.includes(".")) {
        setSuggestions(
          EMAIL_DOMAINS.filter((d) => d.startsWith(afterAt.toLowerCase()))
        );
      } else if (!afterAt) {
    ...
    <div className="w-full space-y-2">
      ...
      <div className="flex justify-center gap-1.5 max-w-[420px] mx-auto">
        <button
          ...
          className="px-4 h-[42px] rounded-lg bg-white/15 text-white font-semibold text-sm hover:bg-white/25 transition-colors"
        >
          {showNumbers ? "ABC" : "123"}
        </button>
        <button
          onClick={() => press("@")}
          className="w-[42px] h-[42px] rounded-lg bg-white/15 text-white font-bold text-lg hover:bg-white/25 transition-colors flex items-center justify-center"
        >
          @
        </button>
        <button
        ...
        <button
          onClick={() => press(" ")}
          className="flex-1 h-[42px] rounded-lg bg-white/15 text-white font-semibold text-sm hover:bg-white/25 transition-colors"
        >
          espaço
        </button>
        <button
          onClick={backspace}
          className="w-[50px] h-[42px] rounded-lg bg-white/15 text-white hover:bg-white/25 transition-colors flex items-center justify-center"
        >
          <Delete size={20} />
        </button>
        <button
          onClick={onConfirm}
          className="px-4 h-[42px] rounded-lg bg-orange-500 text-white font-semibold text-sm hover:bg-orange-600 transition-colors flex items-center justify-center gap-1"
        >
...
const SpinningWheel = ({
  ...
  // Find the index of the 1000-point prize to calculate initial offset
  ...
  useEffect(() => {
    ...
    prizes.forEach((prize, i) => {
      ...
      // ── Radial text (along radius, reading outward) ──
      ctx.save();
      ctx.translate(center, center);
      const midAngle = startAngle + (endAngle - startAngle) / 2;
      ctx.rotate(midAngle + Math.PI / 2); // rotate so "up" points outward along radius

      // Now Y axis: negative = toward outer edge
      // Layout from outer edge to center:
      // For 1000+: Crown → Number → "PONTOS"
      // For others: Star → Number → "PONTOS"
  ...
  return (
    <div className="relative inline-block">
      ...
      <div className="rounded-full p-3 bg-white shadow-[0_0_60px_rgba(255,255,255,0.15)]">
        {/* Pointer at top — larger, with subtle shadow */}
        ...
        {/* Center logo circle — white border, dark gradient bg */}
        <div
...
const Roulette = () => {
  ...
  const handleStart = async () => {
    ...
    if (existing) {
      if ((existing as any).status === "redeemed") {
        // Cenário C: já jogou E já resgatou → bloquear
        setError("Você já utilizou sua participação. Obrigado!");
        setChecking(false);
        return;
      }
      // Cenário B: já jogou mas NÃO resgatou → recuperar QR
    ...
    // Cenário A: nunca jogou → permitir giro
    setChecking(false);
    setStep("wheel");
  };
  ...
  const spinWheel = async () => {
    ...
    // Calculate initial offset (same as SpinningWheel component)
    const idx1000 = prizes.findIndex((p) => p.value >= 1000);
    const initialOffset = idx1000 >= 0 ? -(idx1000 * segAngle + segAngle / 2) : 0;

    // Calculate rotation to land pointer in the center of the selected segment.
    // The wheel's visual angle = rotation + initialOffset.
    // We need (rotation + initialOffset) mod 360 === 360 - segCenter
    ...
    const { error: insertError } = await supabase.from("roulette_spins").insert({
      email: trimmedEmail,
    ...
    if (insertError) {
      console.error("Error saving spin:", insertError);
    }
  ...
  return (
    <div className="min-h-screen bg-cover bg-center bg-no-repeat flex flex-col items-center overflow-hidden select-none" style={{ backgroundImage: "url('/images/roulette-bg.png')" }}>
      {/* Logos */}
      <div className="flex items-center justify-center gap-4 pt-8 pb-4">
        {cfg("logo_primary") ? (
          <img src={cfg("logo_primary")} alt="Logo" className="h-12 object-contain" />
        ) : (
          <span className="text-white font-bold text-xl">CommPass</span>
        )}
        {cfg("logo_secondary") && (
          <>
            ...
            <div className="relative z-10 w-full space-y-4">
              ...
              >
                {checking ? (
                  ...
                  cfg("button_text", "ACESSAR ROLETA")
                )}
              </button>
            </div>
          </div>
      ...
      {step === "result" && wonPrize && (
        <div className="flex-1 flex flex-col items-center justify-center w-full max-w-md px-6 pb-8 text-center">
          {recoveredSpin && (
            <div className="bg-amber-500/20 border border-amber-400/40 rounded-xl px-4 py-3 mb-4">
              <p className="text-amber-200 text-sm font-semibold">
                Você já possui um prêmio pendente de resgate
              </p>
              <p className="text-amber-200/70 text-xs mt-1">
                Resgate seu prêmio escaneando o QR Code abaixo no aplicativo.
              </p>
            </div>
          )}
          ...
          <p className="text-white text-6xl font-black mb-1">
            {wonPrize.value >= 1000
              ? wonPrize.value.toLocaleString("pt-BR")
              : wonPrize.label}
          </p>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="px-6 py-3 rounded-lg bg-white/15 text-white font-bold text-sm hover:bg-white/25 transition-colors"
            >
              {cfg("close_button", "FECHAR")}
            </button>
            <button
              onClick={resetSession}
              className="px-6 py-3 rounded-lg bg-white/15 text-white font-bold text-sm hover:bg-white/25 transition-colors flex items-center gap-2"
            >
              <RotateCcw size={16} />
