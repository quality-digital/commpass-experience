...
const HomePage = () => {
  ...
  const upcomingMissions = MISSIONS.filter((m) => !user.completedMissions.includes(m.id)).slice(0, 3);
    ...
    <AppLayout>
      ...
      <div className="px-5 mb-4">
        <motion.div
          ...
          className="p-5 rounded-2xl gradient-hero"
        >
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">{user.role || "participante"} · {user.company || "VTEX Day"}</p>
            </div>
            <div className="flex items-center gap-1.5 px-4 py-2 rounded-full gradient-points">
              <Zap size={16} className="text-primary-foreground" />
              <span className="text-lg font-bold text-primary-foreground">{user.points} pts</span>
            </div>
          </div>
        </motion.div>
      </div>
      ...
      <div className="px-5 mb-4">
        <button
          ...
          className="w-full p-4 rounded-2xl bg-card shadow-card flex items-center justify-between"
        >
