...
const ConfigTab = () => {
  ...
  const handleToggle = async (item: ConfigItem, checked: boolean) => {
    setSaving(item.id);
    await supabase.from("roulette_config").update({ value: checked ? "true" : "false" }).eq("id", item.id);
    toast({ title: `"${item.description || item.key}" ${checked ? "ativado" : "desativado"}!` });
    setSaving(null);
    load();
  };
  ...
  const booleanItems = items.filter((i) => i.type === "boolean");
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      {booleanItems.length > 0 && (
        <>
          <h3 className="font-semibold text-foreground mt-8">Opções</h3>
          <div className="space-y-3">
...
const SpinsTab = () => {
  ...
  const handleDelete = async (spin: Spin) => {
    if (!confirm(`Excluir o QR Code de ${spin.email}?`)) return;
    setDeleting(spin.id);
  ...
  const handleDeleteAll = async () => {
    if (!confirm(`Excluir TODOS os ${spins.length} QR Codes? Esta ação não pode ser desfeita.`)) return;
    setLoading(true);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {spins.length > 0 && (
          <Button size="sm" variant="destructive" onClick={handleDeleteAll} className="gap-1 ml-auto">
